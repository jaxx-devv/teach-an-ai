...
/**
 * Builds the boxy diamond head by taking a RoundedBox-style geometry and
 * tapering its x/z extent per vertex based on height, so the silhouette
 * reads as a kite/diamond (wide cheeks, pointed crown, rounded chin)
 * instead of a sphere or a radially-symmetric lathe.
 */
...
function Face({
  ...
  useFrame(({ clock }) => {
    ...
    // idle wander + cursor tracking combined
...
function MascotBody({ expression }: { expression: MascotExpression }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    // expression transitions, interpolated rather than snapped
    ...
    // random blink instead of a fixed metronome cycle
    ...
    // secondary motion: head leads, ears lag behind, antenna lags most
    // with a light spring overshoot
    ...
    if (antennaGroup.current) {
      // simple damped spring toward targetY for a bit of overshoot/bounce
      const stiffness = 90;
      const damping = 9;
