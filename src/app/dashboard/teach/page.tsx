...
import { getRecentConversationsFor, saveCustomTopicTitleFor, renameTopicFor, deleteConversationFor, type Identity } from "@/lib/identity";
...
import type { ConversationSummary } from "@/lib/guest-db";
...
export default function TeachHomePage() {
  ...
  useEffect(() => {
    if (!identity) return;
    (async () => {
      setConversations(await getRecentConversationsFor(identity, 200));
  ...
  const confirmRename = async (newTitle: string) => {
    ...
    await renameTopicFor(identity, menuTarget.lessonId, newTitle);
    setIdentity({ ...identity, customTopics: { ...identity.customTopics, [menuTarget.lessonId]: newTitle } });
    setMenuTarget(null);
  ...
  const confirmDelete = async () => {
    if (!identity || !menuTarget) return;
    await deleteConversationFor(identity, menuTarget.lessonId);
    setMenuTarget(null);
    await refetchConversations();
  };
  ...
  const startCustom = async () => {
    ...
    try {
      ...
      // Save the AI-resolved title now, synchronously, while we already
      // have identity loaded. This is what guarantees the clean title
      // (not the raw slug) shows up everywhere later, with no race.
      await saveCustomTopicTitleFor(identity, slug, data.title);
      setIdentity({ ...identity, customTopics: { ...identity.customTopics, [slug]: data.title } });
