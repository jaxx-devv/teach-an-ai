...
export default function TeachSessionPage() {
  ...
  // Resolve the lesson from, in order: the static catalog, a fresh custom-topic
  // query param (first visit right after creation), or the identity's saved
  // custom topic title (returning visits with no query param). This is what
  // makes navigating straight to /dashboard/teach/custom-xyz work.
  ...
  useEffect(() => {
    ...
    if (identity && lesson) {
      if (isFreshCustom) saveCustomTopicTitleFor(identity, lesson.id, lesson.title);
      setCurrentLessonFor(identity, lesson.id);
    }
  ...
  const submitExplanation = async () => {
    ...
    await logMessage(identity, { lessonId: lesson.id, role: "user", content });
    ...
    try {
      ...
      // handled below by falling through to the guaranteed follow-up
    }
    ...
    await logMessage(identity, { lessonId: lesson.id, role: "mascot", content: mascotReply });
    ...
    if (understood) {
      ...
      await completeLessonFor(identity, lesson.id, lesson.xpReward);
      setIdentity({
        ...identity,
        xp: identity.xp + lesson.xpReward,
        lessonsCompleted: identity.lessonsCompleted.includes(lesson.id)
          ? identity.lessonsCompleted
          : [...identity.lessonsCompleted, lesson.id],
      });
  ...
  const markUnderstood = async () => {
    ...
    await logMessage(identity, { lessonId: lesson.id, role: "mascot", content: closingLine });
